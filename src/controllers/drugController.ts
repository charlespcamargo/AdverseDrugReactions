 ...
 *       - name: pageSize
 *         in: query
 *         description: The page size number (default is 10 - The OpenFDA limit is 1000)
 *         required: true
 *         schema:
 *           type: integer
 *     responses:
...
const getAllAdverseReactions = [
    ...
    query('pageSize')
        .optional()
        .trim()
        .isInt({ min: 1, max: 1000 }).withMessage('The page size must be a positive integer between 1 and 1000'),
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            const { drugName, currentPage, pageSize } = req.query;
            const drugResult: Drug = await drugService.getAllAdverseReactions((drugName as string), Number(currentPage) || 1, Number(pageSize));
